...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError('Invalid email or password. Try client@demo.com / demo');
    }
  };
  ...
  const handleForgotPassword = async () => {
    ...
    try {
      ...
      // response.data contains { message, token, email }
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-amber-50 dark:from-slate-950 dark:via-slate-900 dark:to-amber-950/20">
      <div className="flex min-h-screen">
        ...
        <div className="flex-1 flex items-center justify-center p-8">
          ...
          >
            <Link to="/" className="text-sm text-slate-500 dark:text-slate-400 hover:text-amber-500 transition-colors mb-8 inline-flex items-center gap-1">
              ← Back to home
            </Link>
            
            <div className="mb-8">
              <h1 className="text-4xl font-display font-bold text-slate-900 dark:text-white mb-2">Welcome back</h1>
              <p className="text-slate-600 dark:text-slate-400">Sign in to manage your loyalty cards and rewards.</p>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full pl-10 pr-4 py-3 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-900 dark:text-white placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-amber-500 transition-all"
                    required
                  />
                </div>
              </div>
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    placeholder="••••••••"
                    className="w-full pl-10 pr-10 py-3 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-900 dark:text-white placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-amber-500 transition-all"
                    required
                  />
                  <button 
                    type="button" 
              ...
              <div className="flex items-center justify-between text-sm">
                <label className="flex items-center gap-2 text-slate-600 dark:text-slate-400">
                  <input type="checkbox" className="rounded border-slate-300 dark:border-slate-600 text-amber-500 focus:ring-amber-500" /> 
                  Remember me
                </label>
                <button 
                  type="button" 
            ...
            <p className="text-center text-sm text-slate-500 dark:text-slate-400 mt-6">
              ...
              <Link to="/register" className="text-amber-500 hover:text-amber-600 font-medium hover:underline">Create one</Link>
            </p>
          </motion.div>
        </div>

        {/* Right Side - Hero Image */}
        <div className="hidden lg:block lg:w-1/2 relative bg-gradient-to-br from-amber-500/20 to-orange-500/20">
          ...
          <div className="absolute bottom-12 left-12 right-12 z-10">
            ...
            <p className="text-base text-white/80">
              Sign in to manage your loyalty cards and rewards.
            </p>
          </div>
        </div>
      </div>
