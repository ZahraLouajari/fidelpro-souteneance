...
export default function ForgotPasswordPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSuccess(t('auth.code_sent') ?? 'Reset code sent! Check your inbox.');
      setTimeout(() => {
    ...
    <div className="min-h-screen flex">
      ...
      <div className="hidden lg:block lg:w-1/2 relative">
        ...
        <div className="absolute bottom-12 start-12 end-12 z-10">
          ...
          <p className="text-base text-white/80">{t('auth.forgot_sub')}</p>
        </div>
      </div>
      ...
      <div className="flex-1 flex items-center justify-center p-8 bg-background">
        ...
        >
          ...
          <p className="text-muted-foreground mb-8">{t('auth.forgot_sub')}</p>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full ps-10 pe-4 py-3 rounded-xl border border-border bg-background text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-primary transition-all"
                  required
                />
              </div>
            </div>
            ...
            >
              {loading
                ? t('auth.sending')
                : <>{t('auth.send_code')} <ArrowRight className="w-4 h-4" /></>
              }
            </button>
          </form>
